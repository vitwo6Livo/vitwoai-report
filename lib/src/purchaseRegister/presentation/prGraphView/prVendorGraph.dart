...
class VendorGraphModel extends ConsumerStatefulWidget {
  const VendorGraphModel({super.key});

  @override
  ConsumerState<VendorGraphModel> createState() => _VendorGraphModelState();
}

class _VendorGraphModelState extends ConsumerState<VendorGraphModel> {
  int touchedIndex = -1;
  ...
  Widget build(BuildContext context) {
    final PRGraphVendor = ref.watch(prGraphVendorProvider);
    return SingleChildScrollView(
        child: PRGraphVendor.when(
      data: (data) {
        return Card(
          color: Colors.white,
              ...
              children: [
                    ...
                    PieChartData(
                      ...
                      sections: List.generate(data.data.length, (index) {
                        return PieChartSectionData(
                          ...
                          title: data.data[index].TotalVendors,
                ...
                if (touchedIndex != -1)
                  Column(
                    ...
                    children: [
                      Text(
                        'Vendors Range : ${data.data[touchedIndex].range}',
                        style: const TextStyle(
                      ...
                      DataTable(
                        ...
                        rows: data.data[touchedIndex].Vendors.map((vendor) {
                          return DataRow(
                            cells: [
                              DataCell(Text(
                              ...
                              DataCell(
                                Text(
                                  '₹${vendor.totalInvoiceValue.toString()}',
                                ),
                              ),
                            ],
                          );
