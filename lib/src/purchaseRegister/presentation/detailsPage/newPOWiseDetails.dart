...
class newPOWiseDetails extends StatelessWidget {
  final List<dynamic> data;
  final int index;

  const newPOWiseDetails({
    super.key,
    required this.data,
    required this.index,
  });
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            _buildSectionHeader(HandText.prPODetailsTitle),
            _buildCard([
              _buildDetailRow(Icons.person, "PO Number", item.vendorName),
              _buildDetailRow(Icons.code, "PO Date", item.vendorCode),
            ]),
            const SizedBox(height: 16),
            _buildSectionHeader(HandText.prPOVendorInfoDetails),
            _buildCard([
              _buildDetailRow(Icons.person, "Vendor Name", item.vendorName),
              _buildDetailRow(Icons.code, "Vendor Code", item.vendorCode),
            ]),
            const SizedBox(height: 16),
            _buildSectionHeader(HandText.prPOVendorOrderQty),
            _buildCard([
            ...
            const SizedBox(height: 16),
            _buildSectionHeader(HandText.prPOVendorTaxAmt),
            _buildCard([
              _buildDoubleColumn([
